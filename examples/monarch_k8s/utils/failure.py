...
if TYPE_CHECKING:
    from ..train_distributed import MonarchKubernetes, Replica
...
class FailureController:
    @staticmethod
    def kill_k8s(scheduler: "MonarchKubernetes") -> None:
        """
        Kills a random replica Kubernetes job
        """
        candidates = [
            mesh_name
        ...
        logger.info(f"[FailureController] Killing Kubernetes job for {selected}")
    ...
    async def execute_failures(
        ...
        while replicas:
            try:
                ...
                try:
                    if last_failure == Failure.KILL_K8S:
                        FailureController.kill_k8s(scheduler)
                    else:
